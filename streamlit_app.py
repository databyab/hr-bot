...
action_col_1, action_col_2, action_col_3 = st.columns([1, 1, 3])
with action_col_1:
    if st.button("Run ingestion", use_container_width=True):
        with st.spinner("Reading PDFs and building the vector store..."):
            result = ingest_documents()

        if result["pages"] == 0:
            st.error("No PDF files found in data/raw.")
        else:
            st.success(
